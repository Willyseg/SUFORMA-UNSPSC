...
# --- ESTILOS CSS PERSONALIZADOS (Diseño de Tarjetas) ---
st.markdown("""
<style>
    .card {
...
def load_data(file):
    ...
    try:
        ...
        if file is None:
            # Usar datos de muestra si no hay archivo
            df = pd.read_csv(io.StringIO(SAMPLE_CSV), sep=';')
        else:
            df = pd.read_csv(file, sep=';', encoding='latin-1') # Fallback encoding común
        
        # Normalizar nombres de columnas (eliminar espacios extra)
        df.columns = [c.strip() for c in df.columns]
        
        # Identificar columnas clave (búsqueda flexible por nombre)
        col_map = {}
        ...
        # 2. COP: "144.703.000" -> 144703000 (int)
        def clean_cop(val):
            if pd.isna(val): return 0
            # Eliminar puntos y espacios para tener el entero puro
...
with st.sidebar:
    ...
    st.info("Nota: Si usas tu propio archivo, asegúrate que las columnas tengan nombres similares a: 'Objeto', 'Codigos UNSPSC', 'Valor SMMLV', etc.")
...
with c1:
    search_codes = st.text_input("Códigos UNSPSC", placeholder="Ej: 11121600, 14111500 (Separa por comas)")
    st.caption("Muestra experiencias que contengan **TODOS** los códigos ingresados.")

with c2:
    search_object = st.text_input("Objeto del Contrato", placeholder="Buscar por palabra clave...")
    st.caption("Busca texto dentro de la descripción del objeto.")
...
# 2. Filtro por Códigos (AND Lógico)
input_codes_list = []
if search_codes and 'codigos' in col_map:
    # Limpiar input: "123, 456 " -> ['123', '456']
    ...
    if input_codes_list:
        def check_codes(row_codes):
            ...
            # Verificar que TODOS los códigos buscados estén en el string de la fila
...
# 3. Ordenamiento (Mayor a Menor SMMLV)
...
# Formato COP con puntos de miles
m3.metric("Presupuesto Total (COP)", f"${total_cop:,.0f}".replace(",", ".")) 

st.divider()

# --- LISTADO DE RESULTADOS (Renderizado estilo tarjeta HTML) ---
...
if filtered_df.empty:
    ...
    for index, row in filtered_df.iterrows():
        # Extracción segura de datos usando el mapa de columnas
        ...
        # Generar HTML para los códigos (resaltando los buscados)
        codes_html = ""
